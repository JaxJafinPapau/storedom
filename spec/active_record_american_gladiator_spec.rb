...
describe "ActiveRecord American Gladiator" do
  ...
  context "The Maze" do
    it "returns all users that have placed an order" do
      ...
      #   user.orders.present?
      # end
      active_users = User.joins(:orders).group(:name).order(:id)
      # Changeable End
  ...
  context "Atlasphere" do
    ...
    it "returns most popular items" do
      ...
      #   Item.find(id)
      # end
      most_popular_items = Item.select("items.id, count(items.id) AS item_count").joins(:orders).group("items.id").order("item_count DESC").limit(2)
      # Changeable Stop
